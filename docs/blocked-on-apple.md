...
| **#9** — 8-char password minimum is invisible and maps to the wrong error | **CLOSED — fully fixed.** PR #73 fixed the error half. The remaining half (a fix commit closing #9, #21, #24, #25, #29, #30) added `constants/auth.ts`'s `PASSWORD_MIN_LENGTH` as the single shared constant referenced by `sign-in.tsx` and `constants/copy.ts` — `supabase/config.toml`'s `minimum_password_length` remains the sole server-side authority and can't be bound to it, so its value must still be changed in the same commit if it ever moves — and proactive helper text now sits under the password field in sign-up mode (`Copy.auth.password.hint`, wired via `nativeID`/`accessibilityLabelledBy`). GitHub issue #9 is closed. |
