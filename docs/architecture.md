...
**A SECOND blocked reason: `zero_pillar_cooldown` (review r8-1).** `blockedReason` is now a union,
and the free zero-pillar cooldown rides the same channel for a specific reason: the client has to
be able to refuse BEFORE it extracts frames on device and uploads several megabytes it is about to
be told to discard. `pace_quota_status` reports it for free accounts only (that is the only tier
`analyze-form` throttles), reading the same rows and the same
`public.pace_zero_pillar_cooldown_seconds()` interval the refusal itself reads, so the warning and
the refusal can never name different times. When both blocks apply the anti-farm cap wins: it is
the longer of the two, so its `blockedUntil` is the only one at which anything will actually work.
`lib/quota.ts` gives each reason its own sentence — they say opposite things about the runner, and
reading the anti-farm one at a runner whose clip was merely unreadable would accuse them of farming
for filming in bad light.
...
from the ledger. `public.pace_zero_pillar_cooldown_seconds()`
(`20260906140000_quota_status_zero_pillar_cooldown.sql`) owns the interval — 15 minutes, in the
database rather than in TypeScript because `pace_quota_status` needs the same number to pre-flight
the block on Home, and a constant duplicated across the two would let Home name a time the server
does not honour. `flow.ts` calls the lookup AFTER the reserve (the only place tier is learned) and
BEFORE the prompt is built, so a
...
| 429 | `zero_pillar_cooldown` | Free only, and the one body that carries an extra field: `{ error, code, retryAfterSeconds }`. A free resubmission arrived inside `pace_zero_pillar_cooldown_seconds()` (15 min) of that account's last zero-pillar result. Pre-flighted by `quota-status`'s `blockedReason`, so reaching this response at all means a stale reading — `app/analyzing.tsx` renders it as its own panel (the server's sentence plus a clock time), never as a failure, and offers no Retry, because a retry reuses the idempotency key and can only come back 409. Refused before the prompt is built, so no model call is made, and the reservation it took is released uncharged with the non-farming `'zero_pillar_cooldown'` reason. Like the throttle above it, not a 402 — nothing here is for sale, it clears on its own, and `retryAfterSeconds` says exactly when. |
