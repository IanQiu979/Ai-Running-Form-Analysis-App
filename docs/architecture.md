...
## API (endpoint status per row — all listed endpoints are built, tested, and DEPLOYED. Base
deploy 2026-07-26; `analyze-form` and `quota-status` redeployed from `main` 2026-09-19 as v18 and
v16, which carry the 2026-09-06 Free-tier real-analysis change, zero-pillar-uncharged, the Free
cooldown, canonical verdict pinning and the structural safety note — `docs/status.md` Known Issue #49;
`analyze-form` redeployed again later that day from `1eba3fc` with `20260919120000` (Free video =
5-frame burst, #89) — Known Issue #50)
...
| `GET /functions/v1/quota-status` | JWT | — | `{ tier, used, limit, remaining, frameCap, isLifetime, periodStart, periodEnd, blocked, blockedReason, blockedUntil }` | **Built, Deno-tested, and DEPLOYED to the live project 2026-07-26** (issue #50, 2026-07-12; deployed with #128); **redeployed 2026-09-19 as v16**, which recognises `blockedReason: 'zero_pillar_cooldown'` from the 2026-09-19 `pace_quota_status` — see "Current" below. **Redeployed again 2026-09-21** with the override-family deletion (#237); behaviour unchanged (Known Issue #52). Drives Home "7 of 10 left" (Pro/Elite, period-based) or "1 of 1 used, lifetime" (Free). `used`/`limit` computed server-side via a new read-only RPC, `pace_quota_status`, that shares `reserve_analysis`'s own `pace_current_period`/`pace_is_farming_signal` calls — never a client counter. `blocked`/`blockedReason`/`blockedUntil` represent issue #6's anti-farm cap as a state independent of quota: a user can have `remaining > 0` and `blocked: true` at the same time. |
...
2026-09-06; deployed 2026-09-19 as `analyze-form` v18).** `pace_current_tier` and its short-circuit are
...
migration written and pushed live 2026-09-19 — `cron.job` `jobid` 2 reads `{"dryRun": false}`,
`docs/status.md` Known Issue #51).** From 2026-08-06 the scheduled body was `{}`,
