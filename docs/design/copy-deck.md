...
| `auth.error.passwordBreached` | "That password has shown up in a data breach before. Pick a different one to keep your account secure." | Sign-up: the submitted password matches a known-breached password. `lib/hibp.ts`'s client-side check is the control of record (server-side is off since 2026-09-12 — see `docs/architecture.md`'s "Current — Supabase config" and `docs/status.md` Known Issue #47); `mapAuthError` (`lib/auth-errors.ts`) shows this string from either path. Blocks account creation; no jargon ("pwned," "hash," "HaveIBeenPwned") and no blaming the user — reused passwords are common, the copy just asks for a different one. |
...
| `upload.error.unsupportedFootage.title` | "This clip won't work for a full analysis" | NEW key (issue #199). `lib/frames.ts`'s `InsufficientFramesError` — the clip decoded, but too few of its frames landed on genuinely different instants to read motion from (low-frame-rate/re-encoded source). Deterministic per clip like budgetExceeded, so no retry CTA — only a way back to choose different footage. Worded for a runner: no "frame rate", no "fps", no "decoder". Mirrored here verbatim from `constants/copy.ts`; not reviewed by `ux-copywriter` or Ian — a draft until certified (`docs/status.md` Known Issue #34). |
| `upload.error.unsupportedFootage.body` | "Its frames are too close to identical to show your body moving between them — that usually means the clip was re-recorded or exported from another app. Try a clip recorded straight from your camera at normal speed." | |
...
| `history.item.deleteCta` | "Delete" | NEW key (issue #55), CERTIFIED by Ian 2026-07-13. This deck specs the confirmation dialog (`history.delete.confirm.*` below) but not a label for the row's own delete trigger — the built screen's affordance is a persistent tappable control per row (design brief §4, screen 8 offers swipe/long-press as alternatives; a persistent tap target reads correctly to VoiceOver with no gesture to discover). |
...
| `paywall.purchase.error.unavailable.body` | "Buying a plan isn't possible in the app yet. Your plan hasn't changed, and nothing was charged." | Rewritten 2026-09-12 (user-audit): the old "This build can't complete an upgrade right now. Check back soon." blamed the build and promised the state would clear on its own; neither is true — there is no in-app purchase in v1 and the dummy endpoint is deliberately off (`docs/status.md` Known Issue #21). Names the outcome, blames nothing, promises nothing. |
...
| `settings.privacy.summary` | "Frames are processed by Anthropic to produce your feedback, the original video never leaves your device, and frames are stored privately until you delete them." | Replaces `settings.privacy.body` and `settings.privacy.deleteNote` (captain's 2026-09-20 polish pass): one sentence carrying the four consent facts — Anthropic processing, video never leaving the device, private storage, removal on delete — with the full disclosure behind the `settings.privacyPolicy.label` link row instead of inline. |
