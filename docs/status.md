...
14. **NEW — Phase 4 (`analyze-form`) contract notes, carried forward from the M1 review.** Not
    ...
    - MUST refuse to run for a user with no recorded consent. `public.consents` (added 2026-07-12,
      ...
      client's consent tick (`<ConsentGate />` then; the sign-up consent row since 2026-09-20) is
      UX only — it can be bypassed by anyone calling the function directly, so it is not the
      control. If this check is skipped, the app processes Art. 9 health data with no legal basis
      and the entire consent record becomes decorative.
