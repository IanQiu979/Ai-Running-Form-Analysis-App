...
describe('analyzingReducer', () => {
  ...
  it('ignores the retired sample event instead of entering a non-persisted terminal phase', () => {
    // Cast keeps this regression test executable after `sample` leaves AnalyzingEvent. A stale
    // event from old hot-reload state must fall through the reducer's default branch.
    const legacySampleEvent = {
      type: 'sample',
      attempt: 1,
      result: mockOutcome.result,
      heroDataUri: 'data:image/jpeg;base64,AAAA',
    } as unknown as Parameters<typeof analyzingReducer>[1];

    expect(analyzingReducer(INITIAL_ANALYZING_STATE, legacySampleEvent)).toBe(INITIAL_ANALYZING_STATE);
  });
