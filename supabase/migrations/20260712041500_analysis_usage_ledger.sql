...
-- Grant the owner-scoped SELECT explicitly rather than leaning on Supabase's
-- legacy auto-expose (`grant all` on every new public table). config.toml notes
-- that default is being removed (cloud projects created after 2026-10-30 no
-- longer auto-expose new tables); without this line, the owner-scoped SELECT
-- policy above would have no underlying grant to act on there and the future
-- Home quota read would fail closed. RLS still scopes it to the owner's rows.
...
begin
  ...
  -- analysis window identically regardless of which period `now()` falls in
  -- at release time. Sourcing from the 'reserved' event also means the
  -- 'released' event is inserted ONLY when its 'reserved' counterpart exists,
  -- which is exactly what keeps count(released) <= count(reserved) per
  -- analysis and stops `used` from ever going negative. If the 'reserved'
  -- event is somehow absent (should never happen — reserve_analysis always
  -- inserts one before returning, and nothing else can reach 'reserved'
  -- status), this select inserts nothing and the analyses row is still
  -- released successfully. That fails CLOSED: the analysis contributed 0 to
  -- the reserved count, so contributing 0 to the released count leaves it
  -- consistent; the worst case is an analysis that stays counted as consumed
  -- (usage higher than intended), never an under-count that could over-grant.
  insert into public.analysis_usage (user_id, analysis_id, event, tier_at_run, reserved_at)
